...
export const linkCommand = new Command()
    ...
    .action(async (filePath, dashboardId, options) => {
    ...
    const existingId = dashboard[idField];
    if (existingId) {
        console.error(`Warning: Dashboard already has ${idField}: ${existingId}`);
    ...
    if (!result.exists) {
        ...
        console.error("Make sure DATADOG_API_KEY and DATADOG_APP_KEY are set correctly.");
        process.exit(1);
    }
    console.log(`  Found: ${result.data?.title}`);
    dashboard[idField] = dashboardId;
